...
Using a variational lower bound criterion might yield a suboptimal solution with respect to the true log-likelihood.  
How can we maximize exact log-likelihood?  
If we make $$D_{KL} \left (q_{\phi}(z \mid x) \parallel p_{\theta}(z \mid x) \right ) = 0$$, then the variational lower bound is same as log-likelihood.  
→ design inference network as inverse of generator.  
...
Stochastic gradient update the $$x_{miss}$$ with $$x_{miss, \ i+1} = x_{miss, \ i} + \alpha_i (\frac{\partial \mathrm{log}(p_X((x_{obs}, x_{miss, \ i})))}{\partial x_{miss, \ i}} + \epsilon)$$  