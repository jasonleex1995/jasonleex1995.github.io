...
> **버전: v1.1 정합판.** 정본 `CANON.md` **v1.1**을 인용해 전면 개정했다. v1.0 기준으로 쓰인 이전 판본의 값 중 **정본 §18.3이 지목한 11건은 전부 폐기**되었으며, 그 자리는 정본 인용으로 대체되었다.
>
...
>
> ★ **정본이 소유하며 이 섹션이 정하지 않는 것 (v1.1에서 확정)**: **`dpsRef` 곡선**(§13.5) · **보스 HP 전량**(§13.6) · **`bossHpScale`**(§13.6.1) · **`armorCoreRatio` φ**(§8.13.1) · **`optionalPartCoreRatio`**(§13.6.4) · **`uptimeRef`**(§10.4.3) · **`stages.formations` 파라미터**(§9.9.2) · **전 `certify` 지표의 측정 정의**(§13.1.1). 이전 판본이 이것들을 스스로 정했던 것이 라운드 2의 최대 결함이었다.
>
...
>
> **남은 「정본 추가 요청」은 §14에 3건뿐**이며 **전부 minor**다 — 이전 판본의 17건(blocker 4 포함)은 **정본 v1.1이 전수 심사해 17건 전부 채택**했다(§19.3). 반영 결과는 §14.1의 대조표에 있다.
...
| ★ **S24** `Σ(armor hp) == core.hp × armorCoreRatio` (±1%) · 선택 부위 == armor 1개 × `optionalPartCoreRatio` (±5%) | 정본 §13.4 | **§10.1** |
| `capHits == 0` | 정본 §13.1 | **§12.2** (정본 §12.1의 A층 **80** / B층 **96**으로 재계산) |
...
★ **v1.1이 이 자리에 추가한 두 곡선 — 둘 다 정본 소유이며 이 섹션은 인용만 한다**

| 곡선 | s1 | s2 | s3 | s4 | s5 | s6 | 출처 |
|---|---|---|---|---|---|---|---|
| **`dpsRef`** (명목 · 단일표적 · 무속성 · 보스전 진입 시점) | 50 | 90 | 195 | 335 | 485 | **708** | **정본 §13.5** |
| `× uptimeRef 0.60` = **실효** | 30.0 | 54.0 | 117.0 | 201.0 | 291.0 | **424.8** | 정본 §10.4.3 |
| **`bossHpScale`** (`tier ∈ {stage, mid}`) | 1.0 | 1.8 | 4.4 | 7.5 | 11.9 | (17.3) | **정본 §13.6.1** |
| `bossRamp` (= `bossHpScale ÷ dpsRef 배수`) | 1.00 | 1.00 | 1.125 | 1.125 | 1.222 | — | 정본 §13.6.1 |

- ★ **이전 판본의 「참조 화력 곡선 45/85/150/240/360/520」은 전량 폐기**(정본 §18.3-1). 그것은 **가정**이었고, 정본의 곡선은 **03의 실제 무기 수치**에서 산출된다. 그리고 **대립의 진짜 원인은 단위였다** — 04의 520은 *실효*, 03의 708은 *명목*이며 `708 × 0.60 = 425`가 같은 종류의 수다. **`uptimeRef`를 정본이 소유하므로 이 대립은 재발할 수 없다**(정본 §10.4.3).
- ★ **이전 판본의 `bossHpScale` 제안값 `[1.0, 1.9, 3.4, 5.6, 8.4, 11.5]`도 폐기** — **필요성의 진단은 채택**되었고(테마 셔플 → `manta`가 s1에도 s5에도 온다) **값만 정본이 재산출**했다(정본 §19.3 R-1: 수정채택).
- ★ **`tier: "final"`에는 `bossHpScale`을 적용하지 않는다** — `bossHpScale`은 **셔플 때문에 존재**하고 `tetrarch`는 셔플되지 않는다. 예외가 아니라 **규칙의 정의역**이다(정본 §13.6.1).
- ★ **`bossHpScale[6] = 17.3`은 계산되지만 스테이지 보스에는 쓰이지 않는다** — 스테이지 6에는 `tier: "stage"` 보스가 오지 않는다. **중간보스는 사용한다**(§7.2).
...
>
> ★ **`chaffXpRef = 2` (이 섹션이 확정하는 앵커)**: 정본 §8.10의 `swarmXp = chaff XP × 0.5`와 §8.9의 `중간보스 xp = chaff XP × 25`가 **둘 다 "chaff XP"를 참조**하는데, `chaff` 밴드에는 `drifter`(2)·`spitter`(3)·`rearDart`(3)·`swarmChaff`가 공존한다 → **기준 개체를 정하지 않으면 두 파생식이 값을 갖지 못한다.** 앵커 = **`drifter` = 2** (밴드의 기준 개체: 아무것도 하지 않는 순수 XP 사료). → **`swarmXp = 1`**(§8) · **중간보스 `xp = 50`**(§7.2). 사격하는 chaff(`spitter`·`rearDart`)의 3은 **저작 편차**이며 앵커가 아니다. → §14 **N-1**(표기 명시 요청, minor).
...
★ **새떼 XP = 1 의 산출 (정본 §8.10의 파생식)**: `swarmXp = chaffXpRef(2) × 0.5 = **1**` → `swarmChaff.xp = swarmLancer.xp = 1`. 이전 판본의 **2는 폐기**한다 — 그것이 §8.4의 「새떼 = 스테이지 총 XP의 47%」(정본 §8.10의 ~25% 위반, **S22 위반**)를 만든 두 원인 중 하나였다. 나머지 하나(**웨이브 총 개체 수 부족**)는 §5가 닫는다.
...
→ **최악 +2.7%p ≤ 5%p** ✔ 그리고 **조우 개체 88기**는 정본 §13.2-⑪의 코인 저작 계약(**「웨이브당 ~7기 × 11~14웨이브 ≈ 90기」**)과 일치한다 ✔
...
| 항목 | 정본 §13.2-⑪의 계약 | **이 섹션의 실측** | 차이 |
|---|---|---|---|
| 스테이지당 조우 개체 | ~90기 (웨이브당 ~7기 × 11~14웨이브) | **66~88기** (테마·스테이지별, `sea` s3~5 = **88기**, `bog` s2 = **66기**) | −0~27% |
| `turret`+`bruiser` 조우 | 20% × 90 = 18기 → `× 0.15 × 평균 1.5` = **코인 4** | 15~20% → **코인 3.2~4.0** | **−0.8 ~ 0** |
| 런 총수입 | 228 | **224~228** | −1.8% |
| `medianEndCoins ∈ [0, 120]` | ~78 | **~74~78** | ✔ 밴드 중앙 |

→ ★ **웨이브 코인은 스테이지 수입의 9~14%뿐**(나머지는 보스 16~18 + 중간보스 5~10 + 엘리트 3~13)이므로 −0.8의 부족은 총수입의 **1.8%**다. `medianEndCoins`는 밴드 한복판에 있고, **하한에 붙어 있는 것은 `medianPurchasesPerVisit`(1.0)**이다(정본 §13.2-⑪) → **첫 조정 손잡이는 정본이 확정한 `elite.coin`이며 이 섹션은 그것을 만지지 않는다.** 조우 개체 수 계약의 수치 갱신은 §14 **N-2**(minor).
...
**단일 몸체·부위 없음·3종 전 테마 공용.** `midBossCount`(1~2) · `midBossAtSec` `[35]`/`[30,70]` · ★ **`hp = bosses[].hp × bossHpScale[stage]`** · **`midBossLeaveAfterSec = 30`**(= "선택적"의 정의) · `midBossElementRule = "notThemeAndNotNormal"`(런타임 주입) · `midBossForcedLeaveOnCrisis` · `midBossSummonsAllowed = "mbNest"만` · 보상 `xp` = chaff XP × 25 / `coin` **5** / `healDropChance` **0.35** — ★ **거처 = `bosses[]` 개체 필드**(R-10 채택). (정본 §8.9)

★ **HP 스케일 정정: `enemyHpScale` → `bossHpScale` (정본 §8.9 · §18.3-7)**

이전 판본은 정본 v1.0을 따라 `enemyHpScale`을 썼다. **정본 v1.1이 그것을 자기 결함으로 정정**했다:

```
enemyHpScale[5] = 4.5   vs   bossHpScale[5] = 11.9   vs   플레이어 화력 성장 = 9.7배 (dpsRef)
mbNest(1100 base)를 enemyHpScale로 재면:  1100 × 4.5 = 4,950  →  10.5 게임초에 죽는다
→ 정본 §8.9가 확정한 「DPS 체크 — 보스전 예고편」이 소멸한다 (leaveAfterSec 30의 1/3)
```
- ★ **base 값(900 / 750 / 1100)은 그대로 유효하다** — 정본이 스케일만 바꿨다(정본 §18.3-7).
...
| ★ `bosses[].summon` = **`{archetypeId, count, everySec, formationId}` \| `null`** | **`tier == "mid"` 이고 `midBossSummonsAllowed` 통과 시에만 non-null**, 그 외 = 로드 실패 (정본 §8.9 · §9.8) | **S17** |
...
- **`mbNest`의 `summon`은 `tier:"mid"` + `midBossSummonsAllowed`("mbNest"만) 이중 통과** ✔ S17이 동치로 강제하므로 **`mbHammer`·`mbLancer`의 `summon`은 반드시 `null`**이다(위 §7.2 표의 "—" = `null`).
...
### 7.5 ★ DPS 체크 검산 — `bossHpScale`이 「예고편」을 전 스테이지에서 살린다

`실효 HP = hp × bossHpScale[i]` · `effectiveDps = dpsRef[i] × uptimeRef(0.60) × m` (정본 §10.4.3). 균형 빌드 `m`(정본 §13.6.2) = `[1.35, 1.35, 1.43, 1.53, 1.62, 1.62]`.

| 중간보스 | base | s1 (`×1.0`) | s3 (`×4.4`) | s5 (`×11.9`) | vs `leaveAfterSec` **30** |
|---|---|---|---|---|---|
| `mbHammer` | 900 | 900 ÷ (30×1.35) = **22.2s** | 3,960 ÷ (117×1.43) = **23.7s** | 10,710 ÷ (291×1.62) = **22.7s** | ✔ 전 스테이지 22~24s |
| `mbLancer` | 750 | **18.5s** | **19.7s** | **18.9s** | ✔ 전 스테이지 18~20s |
| **`mbNest`** | 1,100 | **27.2s** | **28.9s** | **27.8s** | ★ **전 스테이지 27~29s = 아슬아슬** |

- ★ **`mbNest`가 30초 직전에 죽는 것이 전 스테이지에서 유지된다** = 정본 §8.9의 「DPS 체크 — 보스전 예고편」이 **선언이 아니라 산술**이 된다. `enemyHpScale`이었으면 s5에서 10.5초였다.
- **왜 스테이지에 거의 무관한가**: `bossHpScale[i] ÷ dpsRef[i]배수 = bossRamp[i]`(1.00~1.222)만 남기 때문이다 — 정본 §13.6.1의 상쇄가 중간보스에도 그대로 작동한다. **테마 셔플 하에서 "몇 번째 스테이지에 왔나"가 중간보스의 난이도를 바꾸지 않는다** = 정본 §13.2-⑨의 `noDeadLuck.worstThemeOrder`가 성립하는 이유와 같은 구조.
- **속성이 얹히면**: `midBossElementRule`이 `{counter, prey}` 중 택1을 주입한다 → `prey`가 뽑히면 테마 스탠스로는 **×0.5 = 시간 2배 = 절대 못 잡는다** → **Q(×1)나 올바른 스탠스로 바꿔야 30초 안에 든다.** 위 표는 `m` = 균형 빌드 기준이며, **스탠스를 안 바꾸면 표의 시간이 무의미해지는 것이 이 개체의 설계 목적**이다.
- 정본 §8.9가 인쇄한 `mbNest` s5 = **26.9s**와 위 27.8s의 차 = `m` 반올림(1.62 vs 1.67)뿐이며 **결론(30초에 아슬아슬)은 동일**하다.
...
**정본 §18.3-9의 통보**: 「새떼 XP = 스테이지 총 XP의 **47%**」는 **위반이다.** 정본 §8.10의 **~25%가 확정**이고 **S22가 ≤ 0.30을 강제**한다. → **웨이브 편성의 총 개체 수를 늘려야 한다.**

**위반의 두 원인과 처분**

| # | 원인 | 처분 |
|---|---|---|
| 1 | `swarmChaff.xp = 2` | ★ **1로 정정.** 정본 §8.10의 파생식 `swarmXp = chaff XP × 0.5`에 `chaffXpRef = 2`(§2.3)를 대입한 값이다. **2는 애초에 파생식을 만족하지 않았다** |
...
→ **전 테마·전 스테이지 최악 0.148 ≤ 0.30** ✔ 그리고 **총 XP 대비 새떼 지분** = `0.148/(1+0.148)` = **12.9% ~ 0.139/(1.139)` = 12.2%** — 정본 §8.10의 「~25%」보다 **낮은 쪽**이다. **S22가 하드 게이트이므로 그것을 먼저 만족시켰고**, ~25%는 물결표가 붙은 목표다. 새떼를 25%까지 올리고 싶으면 손잡이는 **`crisisTotal`(정본 소유)**이지 `swarmXp`가 아니다.

> ★ **왜 이 위반이 중대했나 (정본 §13.2-⑤의 논거)**: 새떼가 총 XP의 47%면 **위기 세션 하나가 잡몹 페이즈 95초 전체보다 큰 XP원**이 되어 정본 §8.8의 파밍 장치 4개(이탈 소멸 · 전멸 압축 · 고XP 도망자 · 중간보스 병행)가 **전부 무의미해진다** — 하단에 앉아 있다가 마지막 25초만 잘하면 되기 때문이다. **`farmXpRatio ≥ 2.0`(파밍 = 진짜 선택의 증명)이 이 한 값에 물려 있었다.**
...
**★ 테마 보스 6종의 HP = 전 6종 동일 (정본 §13.6.2)**

| 부위 | **base hp** | 근거 (정본 소유) |
|---|---|---|
| `core` | **815** | `noElementPass` 목표 곡선에서 **역산**된 값 |
| `armor` ×2 | **1,997** 각 (총 3,994) | `armorCoreRatio` φ = **4.90** → `815 × 4.90 = 3,994` |
| 선택 부위 ×1 | **399** | `optionalPartCoreRatio` = **0.20** × `armor` 부위 1개 (= 부담 없이 포기 가능) |
| **소요 피해** (armor 2 + core) | **4,809** = `5.90 × core` | 선택 부위는 **소요 피해에 없다** — 순수 선택이므로 |
...
**★ 역산의 재현 (정본 §13.6.2의 산술을 이 섹션이 검산한다)**
```
통과 모델: killTime T에 대해 pass(T) = P(u ≥ T/300),  u = 실효가동률 ~ N(0.60, 0.05)
목표 (사용자 확정): noElementPass = [0.90, 0.90, 0.50, 0.50, 0.15]
...
| 규칙 | 값 | 소유 |
|---|---|---|
| `core.hp` | **815** (테마 보스 전 6종) / **5,700** (`tetrarch`, 절대) | **정본 §13.6.4** |
| `armorCoreRatio` φ | **4.90** (a=2) / **13.58** (a=3). R7 밴드 `[0.85·B, B)`, `B = 0.4^-a − 1` | **정본 §8.13.1** |
| 각 `armor` 부위 hp | `core.hp × φ ÷ armorCount` — ★ **armor 부위끼리 균등**(비대칭이면 특화 짝 6종의 대칭이 깨진다) | **정본 §13.6.4** |
| `optionalPartCoreRatio` | **0.20** — 선택 부위 hp = `armor 부위 1개 × 0.20` | **정본 §13.6.4** |
| 목표 격파 시간 | **균형 = `killTimeMedianBalanced` ∈ [110, 155]** → `flow.stagePar[i]` = `180 − 그것` = **25~70** | **정본 §13.6.4** |
| `contactDmg` | 코어 **16** / 부위 **14~15** | 정본 §2.5 저작 가이드 |
| `radius` · `anchor` · `shapeId` · `score` · 시드 이미터 | **이 섹션이 저작** (정본은 HP와 구조만 소유) | **04** |

★ **밸런싱 손잡이의 우선순위 (정본 §13.6.4 — 이 섹션은 어느 것도 만지지 않는다)**: `bossHpScale` → `armorCoreRatio`(R7 밴드 내) → `elementCapTotal`(= 정본 개정). **`.js`를 여는 경우는 없다.**
★ **`check.mjs` S24**가 `Σ(armor hp) == core.hp × armorCoreRatio`(±1%)와 `선택 부위 hp == armor 1개 × optionalPartCoreRatio`(±5%)를 **정적으로 강제**한다 → 이 섹션이 HP를 임의로 적으면 **로드 실패**한다.
...
|---|---|---|---|---|---|---|
| 테마 보스 6종 | 1,997 × 2 = **3,994** | 815 × 4.90 = **3,993.5** | **+0.01%** ✔ | **399** | 1,997 × 0.20 = **399.4** | **−0.10%** ✔ |
...
|---|---|---|---|---|---|---|---|---|---|---|
| `thruster` 스러스터 | `mobility` | **물** | 풀(R) | **399** | 22 | `[0,44]` | `fin` | 14 | 1200 | `speedPxSec × 0.5` **그리고 `ampPx → 0`** = 좌우 왕복 정지 |
| `finL` 좌 지느러미 | `armor` | 불 | **물(E)** | **1,997** | 26 | `[-52,10]` | `fin` | 15 | 1500 | 코어 게이트 1/2 (×0.16 → ×0.40) |
| `finR` 우 지느러미 | `armor` | 풀 | **불(W)** | **1,997** | 26 | `[52,10]` | `fin` | 15 | 1500 | 코어 게이트 2/2 (×0.40 → ×1.00) |
| `core` 몸체 | `core` | 노말 | 아무거나 | **815** | 42 | `[0,0]` | `bulb` | 16 | 8000 | **사망** |

`movePattern: "sway"`, `movePatternParams: {"speedPxSec":34,"ampPx":150,"yHoldPx":170}`, `armorCoreRatio: 4.90`

> ★ **HP는 정본 §13.6.2의 표가 유일한 소유자다** (`core` 815 / `finL` 1,997 / `finR` 1,997 / `thruster` 399). 이전 판본의 `core.hp 4000` · `finL/finR/thruster 900`은 **전량 폐기**다. 그리고 이전 판본이 "정본 §9.8이 직접 저작한 값이며 그대로 인용했다"고 적은 것은 **C-7에 의해 무효** — §9.8의 블록은 **전부 예시**다. `radius`·`anchor`·`shapeId`·`score`·`contactDmg`는 **이 섹션의 저작값**으로 남는다(정본 §13.6.4의 위임).
> **HP 배분이 뒤집힌 것에 주목하라**: 코어 4000 + 부위 2700(코어의 68%) → **코어 815 + armor 3,994(코어의 490%)**. ×1환산 총합은 6,700 → 5,208이지만 **`bossHpScale`이 스테이지 축을 따로 담당**하므로 실효 총량은 오히려 커진다(s5: 5,208 × 11.9 = 61,975).
...
**★ 산술 검증 — 정본 §13.6.2의 상쇄식을 이 보스에 적용** (`killTime = 소요피해 × bossRamp[i] / (30 × m)`)

| 경로 | 스테이지 1 (`bossRamp` 1.00) | 스테이지 5 (`bossRamp` 1.222) | 판정 |
|---|---|---|---|
| **균형 (2/2/2)**, `m` = 1.35 / 1.62 | `3994/1.35 + 815` = 3773 → **125.8s** | `3994/1.62 + 815` = 3280 → **133.6s** | ✔ `killTimeMedianBalanced ∈ [110,155]` |
| **무투자** (`m` = 1.0) | `3994 + 815` = 4809 → **160.3s** | 4809 × 1.222/30 → **195.9s** | `noElementPass` **0.906** → **0.134** ★ |
| **코어 직행** (armor 2, ×0.16) | `815 × 6.25` = 5,094 → **169.8s** | 169.8 × 1.222 → **207.5s** | ★ **s1 성립(≤180) / s5 불가** |
| `thruster` 추가 비용 (풀 스탠스 ×2) | `399/2.0` = 200 → **6.7s** | 6.7 × 1.222 → **8.1s** | 순수 선택 |

> ★ **정본 §13.6.2의 인쇄값과의 ±0.5% 차이 (독립 재검산의 결과 — 은폐하지 않는다)**: 정본은 s5를 `noEl` **196.6s** / `direct` **208.3s** / `bal` **134.1s**로, 이 섹션의 재산출은 **195.9 / 207.5 / 133.6**으로 나온다. **원인은 `core`의 반올림 하나다** — 역산의 세 방정식이 `core = 817.6 / 813.6 / 813.4`를 냈고 정본이 **815로 확정**했으므로(§9.5), 어느 스테이지에서 계산하느냐에 따라 ±0.3%가 남는다. **`pass`로는 0.134 vs 0.145** — 둘 다 `noElementPass[5]`의 밴드 `[0.05, 0.30]` 한복판이다. ★ **정본의 표가 소유자이므로 인쇄값을 따른다**(C-8). 이 차이는 **시뮬이 실측으로 지울 값**이지 문서가 다툴 값이 아니다.

**★ 특화 `4/2/0` 6짝 전수 (스테이지 5, `bossRamp` 1.222)** — `finL` 카운터 = 물, `finR` 카운터 = 불

| (primary 4, secondary 2) | `finL` `m` | `finR` `m` | killTime | pass |
|---|---|---|---|---|
| (물4, 불2) · (불4, 물2) | 2.0 / 1.62 | 1.62 / 2.0 | **124.1s** | ~1.000 |
| (물4, 풀2) · (불4, 풀2) | 2.0 / 1.0 | 1.0 / 2.0 | **155.2s** | 0.951 |
| (풀4, 물2) · (풀4, 불2) | 1.62 / 1.0 | 1.0 / 1.62 | **164.7s** | 0.846 |
| **평균** | | | **148.0s** | **0.932** ≥ 0.85 ✔ |
...
- ★ **코어 직행이 스테이지 1~2에서만 성립하는 것은 버그가 아니라 「소프트 게이트」 그 자체다**(정본 §13.6.2): 169.8s ≤ 180 → 가능하되 **armor를 깨는 것(160.3s)보다 6% 손해.** **무투자 빌드조차 armor를 깨는 것이 이득**이고(R7의 G-1), 스테이지 3부터는 직행이 **물리적으로 불가능**해진다.

### 10.3 「용광로 거신 (Kiln)」 — `volcano` / 불 — **armor 2로 재구성**

| 부위 | `partType` | `element` | **정답 스탠스** | `hp` (base) | `radius` | `anchor` | `shapeId` | `contactDmg` | `score` | 파괴 효과 |
|---|---|---|---|---|---|---|---|---|---|---|
| `turret` 포탑 | `armament` | **불** | 물(E) | **399** | 24 | `[0,-40]` | `slab` | 14 | 1300 | **`laser` 이미터 영구 삭제** |
| `plate` 흉갑 | `armor` | 물 | **풀(R)** | **1,997** | 30 | `[-52,20]` | `ring` | 15 | 2400 | 코어 게이트 1/2 |
| ★ `vent` 배기구 | `armor` | 풀 | **불(W)** | **1,997** | 30 | `[52,20]` | `ring` | 15 | 2400 | 코어 게이트 2/2 · **용암 `zone` 이미터 소멸**(§9.2) |
| `core` 노심 | `core` | 노말 | 아무거나 | **815** | 40 | `[0,0]` | `bulb` | 16 | 8000 | **사망** |
...
| 경로 | 스테이지 1 | 스테이지 5 | 판정 |
|---|---|---|---|
| **균형 (2/2/2)** | **125.8s** | **133.6s** | ✔ |
| **무투자** | **160.3s** | **195.9s** (정본 인쇄 196.6) | `noElementPass` 0.906 → 0.134 |
| **코어 직행** (×0.16) | **169.8s** ✔ 성립(손해) | **207.5s** ✗ 불가 | ★ 아래 |
| `turret` 추가 비용 (물 스탠스 ×2) | **6.7s** | **8.1s** | 순수 선택 |
...
|---|---|---|
| 코어 직행 | `kiln`·`scarab`·`mire`에서만 성립 | ★ **전 보스에서 스테이지 1~2에 성립(169.8s ≤ 180), 스테이지 3부터 불가** |
...
> ★ **"때릴 수는 있다. 하지만 손해다"가 초반이고, "산술적으로 못 죽인다"가 후반이다. 두 문장이 한 곡선 위에 있다.** — 그리고 스테이지 1~2의 직행(169.8s)조차 **armor를 깨는 것(160.3s)보다 6% 손해**다: **무투자 빌드에게조차 armor가 이득**이라는 것이 R7의 G-1이며, **그것이 게이트가 게이트이기 위한 조건**이다.

- ★ **Q(노말 스탠스)의 존재 이유는 여기서도 증명된다** — 화산을 **불+4**로 밀고 온 특화가 `plate`(물)를 **불 스탠스로 때리면 ×0.5**다: `1997/0.5 = 3,994` → 그 부위 하나에 s5에서 **162.7초** = 타이머 초과. **Q로 바꾸면 ×1 = 81.3초.** "모르겠으면 Q"가 **81초짜리 실물 마진**이 된다(이전 판본의 13초보다 크다 — HP가 armor로 옮겨갔기 때문이다).
...
> **HP는 전 6종 동일**(정본 §13.6.2): `armor` **1,997** 각 · 선택 부위 **399** · `core` **815** · `armorCoreRatio` **4.90**. 아래 표는 **속성 배치·이미터·형상**만 다르다 — 그것이 이 섹션의 위임분이다.

| 보스 | 부위 | `partType`/`element` | 정답 | `hp` | `radius` | `anchor` | `shapeId` | `contactDmg` | `score` | 시드 이미터 | 파괴 효과 |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **`frostCrown`** 빙관 (`glacier`/물) `movePattern:"sway"` `{speedPxSec:30, ampPx:140, yHoldPx:165}` | `pylon` 서리탑 | `armament`/불 | 물(E) | **399** | 22 | `[0,-38]` | `cross` | 14 | 1300 | `wall`/`frostBrick`/`count:9, gapCount:1, gapWidthPx:84, speed:105` ★ **페이즈 3 = `stunMark`** | 얼음 벽 영구 삭제 |
| | `crownL` 좌관 | `armor`/풀 | **불(W)** | **1,997** | 26 | `[-56,16]` | `spike` | 15 | 2400 | `straight`/`pelletS`/`count:4, spreadDeg:34, speed:150` | 게이트 1/2 |
| | `crownR` 우관 | `armor`/불 | **물(E)** | **1,997** | 26 | `[56,16]` | `spike` | 15 | 2400 | `spiral`/`pelletS`/`count:8, speed:140, rotStepDeg:24, durationSec:2.0, rateSec:0.25` | 게이트 2/2 |
| | `core` | `core`/노말 | 아무거나 | **815** | 40 | `[0,0]` | `orb` | 16 | 8000 | — (코어는 사격하지 않는다) | **사망** |
| **`scarab`** 태양갑충 (`desert`/불) `movePattern:"orbitArc"` `{speedPxSec:38, ampPx:130, yHoldPx:150}` | `carapace` 갑각 | `armor`/물 | **풀(R)** | **1,997** | 30 | `[-50,26]` | `slab` | 15 | 2400 | `fan`/`fanShard`/`count:7, arcDeg:96, speed:130` | 게이트 1/2 |
| | ★ `stinger` 침포 | `armor`/풀 | **불(W)** | **1,997** | 30 | `[50,26]` | `spike` | 15 | 2400 | `aimed`/`homingM`/`count:2, spreadDeg:20, speed:90, leadSec:0.35` | 게이트 2/2 · **유도탄 소멸** |
| | `legs` 다리마디 | `mobility`/**불** | 물(E) | **399** | 20 | `[0,44]` | `claw` | 14 | 1200 | `straight`/`pelletS`/`count:3, spreadDeg:20, speed:150` | `speedPxSec × 0.5` + `ampPx → 0` = **호 기동 정지** |
| | `core` | `core`/노말 | 아무거나 | **815** | 38 | `[0,0]` | `hexPod` | 16 | 8000 | — | **사망** |
| **`thornKing`** 가시왕 (`forest`/풀) `movePattern:"sway"` `{speedPxSec:32, ampPx:150, yHoldPx:170}` | `podL` 좌 꼬투리 | `armor`/물 | **풀(R)** | **1,997** | 25 | `[-54,12]` | `orb` | 15 | 2400 | `ring`/`fanShard`/`count:9, speed:130, rotOffsetDeg:20` | 게이트 1/2 |
| | `podR` 우 꼬투리 | `armor`/불 | **물(E)** | **1,997** | 25 | `[54,12]` | `orb` | 15 | 2400 | `straight`/`pelletS`/`count:5, spreadDeg:40, speed:150` | 게이트 2/2 |
| | `bloom` 화포 | `armament`/**풀** | 불(W) | **399** | 26 | `[0,-34]` | `ring` | 14 | 1300 | `wall`/`thornBrick`/`count:8, gapCount:1, gapWidthPx:92, speed:120` | **가시 벽 영구 삭제** |
| | `core` | `core`/노말 | 아무거나 | **815** | 40 | `[0,0]` | `bulb` | 16 | 8000 | — | **사망** |
| **`mire`** 늪주 (`bog`/풀) `movePattern:"holdCenter"` `{speedPxSec:0, ampPx:40, yHoldPx:155}` | `shell` 이끼갑 | `armor`/불 | **물(E)** | **1,997** | 30 | `[-50,28]` | `hexPod` | 15 | 2400 | `fan`/`fanShard`/`count:7, arcDeg:110, speed:130` | 게이트 1/2 |
| | ★ `tendril` 촉수각 | `armor`/물 | **풀(R)** | **1,997** | 30 | `[50,28]` | `claw` | 15 | 2400 | `straight`/`pelletS`/`count:3, spreadDeg:24, speed:150` | 게이트 2/2 |
| | `sac` 포자낭 | `armament`/**풀** | 불(W) | **399** | 22 | `[0,-40]` | `orb` | 14 | 1300 | `spiral`/`hexBolt`/`count:10, speed:95, rotStepDeg:26, durationSec:2.0, rateSec:0.2` ★ **페이즈 3 = `stunMark`** | ★ **둔화·스턴 이미터 영구 삭제** |
| | `core` | `core`/노말 | 아무거나 | **815** | 38 | `[0,0]` | `bulb` | 16 | 8000 | — | **사망** |
...
- ★ **`sac`(포자낭) 파괴 = 둔화와 스턴이 동시에 사라진다.** `armament`의 정의가 **런에서 가장 큰 페이오프**가 되는 지점 — 늪주는 "상태이상을 끄고 싸울 것인가, 시간을 아낄 것인가"가 곧 보스전의 결정이다. 그리고 `sac`는 **풀(테마 속성)**이라 늪을 불+4로 밀고 온 빌드가 ×2로 가장 빨리 끌 수 있다 = **테마 특화가 보상받는 유일한 부위**(R4가 1개만 허용하므로). **선택 부위 hp = 399**이므로 그 값은 s5에서 `399 × 11.9 ÷ (291×2.0)` = **8.2초** = 싸다 → "끄는 것"이 **현실적인 선택지**가 된다.
...
★ **최종 로테이션의 시각 규격은 「없다」가 확정이다 (정본 §7.12.3)**: 02가 "서브웨이브마다 띠가 속성색으로 펄스"를 제안했으나 **기각**되었다 — ① 정본 §7.2의 **가장 강한 예약**("적 탄·텔레그래프에 4속성 색 절대 금지") 위반 ② **불필요**하다: **적의 외곽선이 이미 답**이고(§7.6의 <200ms 판독 계약), 서브웨이브 간격 **4.17초**(§8.2의 파생값)는 무-트위치 예산의 7배이며, `stance.switchCooldown = 0`이다. → **"미리 알려주면 시험이 아니라 대본이다."** 이 섹션은 **새떼의 `element`만 저작**하고 시각 채널을 만들지 않는다.
...
> ★ **왜 최종만 절대값인가**: `bossHpScale`은 **셔플 때문에 존재**하고 `tetrarch`는 셔플되지 않는다 → **적용할 이유 자체가 없다.** 그리고 적용하면 `tetrarch`의 base가 `5700/17.3 = 330`이 되어 **`manta`의 코어(815)보다 작아 보인다** = **AI·리뷰어가 읽는 JSON이 거짓말을 한다**(I-2의 정신).
...
- ★ **`noElementPass`에 상한이 생긴 것이 이 게이트의 성격을 바꿨다.** 단일 값 0.55는 **"브릭 없음"만** 증명했고 「필수」를 증명하지 못했다. 곡선 + 상한은 **「초반엔 무투자로도 되고(0.906), 후반엔 속성 투자가 필수(0.149)」를 6개의 숫자로 증명**한다(정본 §13.3). **무투자가 최종 보스를 깨면 그것은 통과가 아니라 실패다.**
...
- ★ **`mbNest`가 뽑히면 테트라크의 관을 미리 재보는 셈이 된다** — 스테이지 6의 `mbNest` = `1,100 × bossHpScale[6](17.3)` = **19,030** → `19,030 ÷ (424.8 × 1.62)` = **27.7초** vs `leaveAfterSec` 30 ✔ **여전히 아슬아슬**하고, 그것이 곧 관 하나(25,800 → 37.5초 @ ×1.62)의 **74% 크기 예고편**이다. `bossHpScale[6] = 17.3`이 "계산되지만 스테이지 보스에는 쓰이지 않는" 값인데도 **배열에 남아 있는 이유가 이것이다**(정본 §13.6.1: "중간보스는 사용한다").
...
| **G3** | 보스 **R1~R7 전부** + `partCount`(코어 포함 4 / finale 5) + ★ **`armor` 수 == 2**(finale 3) + ★ **S24 HP 배분** + ★ **S18 `holdCenter` ⇒ mobility 금지** | S5 · S18 · S24 · §10.1 |
...
```
→ **어디서도 A층 80에 닿지 않고, B층 96의 `deferAttack`은 발동할 일이 없다** → **`capHits = 0`** ✔ (정본 §13.2-⑦의 결론과 일치)
...
|---|---|---|
| `dpsProbe.balancedPass ≥ 0.95` (전 스테이지) | 테마 보스 **125.8~135.7s** / 테트라크 **125.9s** (제한 180) | ✔ **0.998~1.000** §10.2 · §11.4 |
| `dpsProbe.specialistPass ≥ 0.85` | 테마 보스 6짝 평균 **148.0s → 0.932** (최악 짝 164.7s → 0.846) / 테트라크 **142.0s → 0.994** | ✔ **아슬** = 의도 |
| ★ `dpsProbe.noElementPass` **곡선** `min [0.78,0.78,0.35,0.35,0.05,0.05]` / `max [0.98,0.98,0.65,0.65,0.30,0.28]` | **0.906 / 0.906 / 0.478 / 0.515 / 0.134 / 0.149** (§10.2 · §11.4) | ✔ **전 6항 밴드 안** — 사용자 확정 곡선(0.90/0.90/0.50/0.50/0.15)을 산술이 재현 |
| ★ `killTimeMedianBalanced ∈ [110, 155]` | **125.8 ~ 135.7s = 2.10~2.26분** | ✔ 사용자의 "균형 ~2~2.5분" |
| `bossTimeoutRate ∈ [0.02, 0.25]` | 7 보스 전부 균형 125~136s → 타임아웃은 **언더레벨·무투자 런에 집중**. ★ **하한 0.02**: 무투자가 s5·s6에서 **구조적으로 초과**하므로(195.9s / 195.6s vs 180) **타임아웃 사망이 0이 될 수 없다** = 3분 타이머가 **살아 있는 규칙** | ✔ 양 끝 |
...
| `dominance.maxThemeClearStddev ≤ 0.06` | ★ **근거 교체**: armor 1/2 축은 사라졌다(전부 armor 2). 새 근거 = **선택 부위 타입 2:4 · `movePattern` 3:1:2 · Σ XP ±10% 정규화**(§5.6) + `s_t ∈ [0.89, 0.97]` 저작 계약(정본 §13.2-⑨) | ✔ 추정 0.017 |
...
| ★ **S22** 새떼 XP ≤ 0.30 | **최악 0.148**(`forest` s4·s5) — `swarmXp 1` + 블록 4개 편성 | ✔ **여유 2.0배** §8.4 |
| ★ **S23** 코인원 균질성 | 전 6테마 `turret`+`bruiser` = **1~2종** | ✔ §4 |
| ★ **S24** HP 배분 | 테마 보스 ±0.10% / `tetrarch` ±0.78% | ✔ §10.1 |
| `capHits == 0` | 적 ≤ 40 / 탄 ≪ 320 / **텔레그래프 3σ ≈ 13 ≪ A층 80** | ✔ **전제 없음** (R-4 반영 완료) |
...
| ★ **웨이브 수 순환** | 스테이지 1 리스트 = **10 웨이브**(T1a+T1b) < `mobPhaseMaxWaves` 14 → **순환이 실제로 발동**한다 | 11 웨이브 | 14 웨이브 |
...
### 14.0 ★ 이전 판본의 요청 17건 — 정본 v1.1이 **전수 채택**했다

정본 §19.3의 심사 결과: **채택 15 · 수정채택 2 · 기각 0.** blocker 4건 전부 해소되었다. → **이 섹션에 남은 요청은 §14.2의 3건뿐이며 전부 minor다.**

| # | 요청 | 판정 | **정본이 반영한 곳** | 이 문서의 인용 위치 |
|---|---|---|---|---|
| **R-1** | `bossHpScale` 신설 (blocker) | **수정채택** — 필요성은 정확, **값은 폐기**하고 정본이 재산출 `[1.0, 1.8, 4.4, 7.5, 11.9, 17.3]`. `tier:"final"`에는 미적용 | §13.6.1 | §1.4 · §9.5 |
...
### 14.1 ★ 정본이 뒤집은 것 — 이 섹션이 승복한 11건 (정본 §18.3)
...
| 7 | **중간보스 HP 스케일 = `enemyHpScale`** | `bossHpScale`이 옳다 — `enemyHpScale`이면 `mbNest`가 s5에서 **10.5초**에 죽어 「DPS 체크」가 소멸. **base 값 900/750/1100은 그대로 유효** | §7.1 정정 · §7.5 검산 신설 |
| 8 | **`coinScarcity` ⚠ 「조정 손잡이 = 후보 웨이브 수」** | ★ **이 섹션이 가리킨 §6의 제목은 「아무것도 정하지 않는다」였다 = 존재하지 않는 손잡이.** 손잡이 = **`elite.coin`**(정본 소유), 분산은 **S23**이 봉쇄 | §6 전면 재작성 |
| 9 | **「새떼 XP = 스테이지 총 XP의 47%」** | **정본 §8.10의 ~25%가 확정**이고 **S22**가 ≤0.30 강제. → **웨이브 편성의 총 개체 수를 늘려야 한다** | §2.3(`swarmXp 1`) · §5.3(블록 4개) · §8.4 |
...
### 14.2 ★ 남은 정본 추가 요청 — 3건 (전부 minor)

> **전부 「정본의 서술이 이 섹션의 개정으로 낡았다」는 종류**이며, **어느 것도 값·규칙을 바꾸지 않는다.** 반영되지 않아도 `check.mjs`·`--certify`는 통과한다 → **blocker 0.**

| # | 등급 | 항목 | 요청 | 근거 |
|---|---|---|---|---|
| **N-1** | **minor** | **「chaff XP」의 앵커가 미정** | 정본 §8.10(`swarmXp = chaff XP × 0.5`)·§8.9(`중간보스 xp = chaff XP × 25`)의 **"chaff XP" = `chaffXpRef`**임을 명시하고, **그 값이 「적·스테이지」 섹션의 저작물**(= `drifter.xp`)임을 못박을 것 | `chaff` 밴드에 `drifter`(2)·`spitter`(3)·`rearDart`(3)가 공존한다 → **기준 개체를 정하지 않으면 정본의 두 파생식이 값을 갖지 못한다.** 이 섹션은 **`chaffXpRef = 2`**로 확정했고(§2.3) 그 위에 `swarmXp = 1` · 중간보스 `xp = 50`이 선다. **정본이 앵커의 존재를 명시하지 않으면 구현자가 `spitter`(3)를 골라 두 파생식이 조용히 1.5배가 된다**(C-6 위반) |
| **N-2** | **minor** | **정본 §8.7·§13.2-⑤의 예시 수치 「`sea` 스테이지 1의 리스트는 **5 웨이브**」** | **「10 웨이브」로 갱신** (T1a + T1b, §5.5) | **S22가 5웨이브 20기를 금지한다**(`30/50 = 0.60 > 0.30`) → 정본 §18.3-9의 "총 개체 수를 늘려야 한다"를 이행한 결과다. ★ **`waveListExhausted: "cycle"`의 필요성은 그대로다**(소극 파밍 11웨이브 > 10 → 순환이 실제로 발동) → **규칙은 불변, 예시 수치만 낡았다.** 그리고 정본 §8.7의 「최소 리스트 5웨이브 → 최대 2.8회차」도 **「10웨이브 → 최대 1.4회차」**가 된다 |
| **N-3** | **minor** | **정본 §13.2-⑪의 코인 저작 계약 「웨이브당 ~7기 × 11~14웨이브 ≈ **90기**」** | **「~72기 (56~88, 테마별)」로 갱신**하거나, **밴드로 표기**할 것 | §5.6의 Σ XP 정규화(±10%)를 우선하면 **개체 수는 테마별로 갈린다**(chaff의 XP가 높은 `bog`는 개체가 적다). 실측 = **`sea` s3~5 88기 ~ `bog` s2 66기**. → 웨이브 코인 **4 → 3.2~4.0**, 런 총수입 **228 → 224~228 (−1.8%)** → `medianEndCoins`는 밴드 한복판(**~74~78** ∈ [0,120]) ✔ **게이트는 통과하나 계약의 수치가 낙관적이다.** ★ **손잡이는 요청하지 않는다** — 정본이 `elite.coin`으로 이미 소유했다 |

### 14.3 이 섹션이 정본을 바꾸지 **않은** 것 (확인)
...
- **위기 세션의 모든 규칙** — 정본 §8.10 그대로. 이 섹션은 6 서브웨이브의 9:1 분산만 정했다. **`swarmXp`는 정본의 파생값이며 손잡이가 아니다.**
- **코어 게이트·R1~R7·`partType` 4종·`movePattern` 3종·`phaseThresholds`·부위 XP 0** — 정본 §8.11~§8.14 그대로.
- ★ **보스 HP·`armorCoreRatio`·`optionalPartCoreRatio`·`bossHpScale`·`dpsRef`·`uptimeRef`** — **전부 정본 소유**(§13.5·§13.6). 이 섹션은 **인용과 검산만** 한다.
