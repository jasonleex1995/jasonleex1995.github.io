...
suite('difficulty/표 §11.3', () => {
  test('난이도는 셋이고 네 열이 모두 순증한다', () => {
    ...
    assert.eq(d[ids[0]].hpMul, 1, '첫 난이도(노멀)가 기준선 — hpMul 1');
...
suite('difficulty/체력 배율 §11.3', () => {
  ...
  test('네 스포너 전부가 hpMul 을 탄다 — 잡몹·중간보스·보스 코어·보스 파트', () => {
    const data = loadData();
    const ids = tierIds();
    const hi = ids[ids.length - 1];                 // 가장 어려운 난이도
    ...
    for (const [name, spawn] of cases) {
      const base = spawn(mkWorld(ids[0]));
      const hard = spawn(mkWorld(hi));
      assert.eq(base.hp, 1000, `${name}: 기준 난이도는 저작값 그대로`);
      assert.near(hard.hp, 1000 * mul, 1e-6, `${name}: hp × hpMul`);
      assert.eq(hard.hpMax, hard.hp, `${name}: hpMax 도 같이 오른다 (체력바가 거짓말하지 않는다)`);
    }
  });
