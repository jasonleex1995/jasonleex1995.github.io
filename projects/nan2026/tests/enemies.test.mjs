...
suite('enemies · element 편성 주입 (§8.6 — 상성의 핵심)', () => {
  test('첫 웨이브의 element 는 웨이브 레코드가 주입한다', () => {
    const w = mk(7);
    silence(w);
    // sea stage-1 첫 해금 웨이브 = drifter water 16 (scatter). 데이터에서 유도.
    ...
    for (let i = 0; i < items.length; i += 1) {
      ...
      if (!e.alive) continue;
      n += 1;
      assert.eq(e.archetypeId, wave0.archetypeId, '아키타입 = 웨이브 레코드');
      assert.eq(e.element, wave0.element, 'element = 웨이브 레코드 (아키타입 필드 아님)');
    }
    assert.eq(n, wave0.count, `첫 웨이브 count(${wave0.count})만큼 스폰`);
  });
