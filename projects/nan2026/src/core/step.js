...
function movePlayer(world, dt) {
  ...
  if (p.slowSec > 0) {                                                 // §2.7 — 강도는 불변
    ...
    //   ★ tmul > 0 도 함께 본다 — 면역(tmul 0)이면 지형은 타이머를 «아예 안 세운다»(149행). 그러니 그때 남아 있는
    //   둔화는 무조건 탄에서 온 것이고, 저항을 먹이면 안 된다. 이 조건이 없으면 면역인 채 둔화 장판 위에 선 플레이어가
    //   **탄 둔화의 마지막 한 틱**을 잃는다(탄 둔화가 dt 아래로 줄어드는 그 틱에 지형 것으로 오인된다).
    const fromTerrain = tk === T_SLOW && tmul > 0 && p.slowSec <= dt + 1e-9;
